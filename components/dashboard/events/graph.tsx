...
export function EventGraph() {
  ...
  const fetchGraphData = async (selectedPeriod: Period) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/dashboard/events/graph?period=${selectedPeriod}`);
      ...
      console.error('Failed to fetch event graph data:', error);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="flex-1">
            ...
            <CardDescription className="text-xs sm:text-sm mt-1">
              {periodLabels[period]} • Total: {graphData.total.toLocaleString()} events
            </CardDescription>
          </div>
        ...
        {peakPoint.count > 0 && (
          <div className="flex items-center gap-2 text-xs sm:text-sm mt-2">
            ...
            <span className="font-semibold text-primary">
              {peakPoint.date} ({peakPoint.count} events)
            </span>
          </div>
        )}
      </CardHeader>
