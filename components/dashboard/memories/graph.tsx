...
export function MemoriesGraph() {
	...
	const fetchGraphData = async (selectedPeriod: Period) => {
		try {
			setLoading(true);
			const response = await fetch(`/api/dashboard/memories/graph?period=${selectedPeriod}`);
			...
			console.error('Failed to fetch memory graph data:', error);
		} finally {
			setLoading(false);
		}
	};
	...
	if (!graphData || graphData.dataPoints.length === 0) {
		return (
			<Card className="py-4">
				<CardHeader className="pb-2">
					<CardTitle className="text-base sm:text-lg">Memories Created Over Time</CardTitle>
				...
				<div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
					<div className="flex-1">
						...
						<CardDescription className="text-xs sm:text-sm mt-1">
							{periodLabels[period]} • Total: {graphData.total.toLocaleString()} memories
						</CardDescription>
					</div>
				...
				{peakPoint.count > 0 && (
					<div className="flex items-center gap-2 text-xs sm:text-sm mt-2">
						...
						<span className="font-semibold text-primary">
							{peakPoint.date} ({peakPoint.count} memories)
						</span>
					</div>
				)}
			</CardHeader>
