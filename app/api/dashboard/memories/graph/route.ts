...
export async function GET(request: Request) {
	try {
		...
		// Fetch all memories in the date range
		...
		// Group memories by the appropriate time period
		const groupedData = new Map<string, number>();

		for (const memory of memories) {
			const key = formatDate(memory.created_at, groupFormat);
		...
		const dataPoints: MemoryDataPoint[] = allLabels.map((label) => ({
			date: label,
		...
		console.error('Error fetching memory graph data:', error);
		return NextResponse.json(
			{
				success: false,
