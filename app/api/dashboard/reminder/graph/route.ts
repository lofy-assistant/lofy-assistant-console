...
export interface ReminderDataPoint {
  date: string;
  count: number;
}

export interface ReminderGraphData {
...
export async function GET(request: Request) {
  try {
    ...
    // Fetch all reminders in the date range
    ...
    // Group reminders by the appropriate time period
    const groupedData = new Map<string, number>();
    
    for (const reminder of reminders) {
      const key = formatDate(reminder.created_at, groupFormat);
    ...
    const dataPoints: ReminderDataPoint[] = allLabels.map(label => ({
      date: label,
      count: groupedData.get(label) || 0
    }));

    const graphData: ReminderGraphData = {
      dataPoints,
      period,
    ...
    console.error("Error fetching reminder graph data:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to fetch reminder graph data",
